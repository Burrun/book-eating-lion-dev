...
class RecentViewedBookRepositoryTest {
    ...
    void 최근_본_기록을_저장하고_조회한다() {
        recentViewedBookRepository.save(RecentViewedBook.builder()
                .memberId("member-1").book(book1).viewedAt(LocalDateTime.now()).build());

        assertThat(recentViewedBookRepository.findByMemberIdAndBook_BookId("member-1", book1.getBookId())).isPresent();
    }
    ...
    void 최근_본_순으로_조회한다() {
        RecentViewedBook older = recentViewedBookRepository.save(RecentViewedBook.builder()
                .memberId("member-1").book(book1).viewedAt(LocalDateTime.now().minusDays(1)).build());
        recentViewedBookRepository.save(RecentViewedBook.builder()
                .memberId("member-1").book(book2).viewedAt(LocalDateTime.now()).build());

        List<RecentViewedBook> result = recentViewedBookRepository
                .findByMemberIdOrderByViewedAtDesc("member-1", PageRequest.of(0, 10));
