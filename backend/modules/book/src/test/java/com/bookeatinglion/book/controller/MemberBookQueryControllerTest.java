...
class MemberBookQueryControllerTest {
    ...
    void 내_최근_본_책_조회는_200과_데이터를_반환한다() throws Exception {
        when(recentViewedBookService.getMyRecentBooks(eq(1L), eq(20))).thenReturn(List.of(summary(1L, "최근본책")));
    ...
    void 최근_본_책_조회시_limit을_지정할_수_있다() throws Exception {
        when(recentViewedBookService.getMyRecentBooks(eq(1L), eq(5))).thenReturn(List.of(summary(1L, "최근본책")));

        mockMvc.perform(get("/api/recent-books/me").header("X-Member-Id", "1").param("limit", "5"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data[0].title").value("최근본책"));
    }
