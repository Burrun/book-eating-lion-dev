...
class ProductInquiryControllerTest {
    ...
    @MockBean
    CatalogMemberIdentity memberIdentity;
    ...
    void 문의_조회_작성_수정_삭제_경로가_동작한다() throws Exception {
        ...
        when(inquiryService.getBookInquiries(eq(1L), any(), any())).thenReturn(new PageImpl<>(List.of(response)));
        ...
        mockMvc.perform(patch("/api/catalog/inquiries/10")
                ...
                .andExpect(status().isOk());
        mockMvc.perform(delete("/api/catalog/inquiries/10")).andExpect(status().isOk());
    }

    @Test
    void 관리자_문의_목록과_답변_경로가_동작한다() throws Exception {
        when(memberIdentity.requiredMemberId()).thenReturn("admin-sub");
        when(inquiryService.getAdminInquiries(any(), any(), any())).thenReturn(new PageImpl<>(List.of(response())));
