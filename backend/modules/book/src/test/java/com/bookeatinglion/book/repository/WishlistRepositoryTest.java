...
class WishlistRepositoryTest {
    ...
    void 찜을_저장하고_회원_책_조합으로_조회한다() {
        wishlistRepository.save(Wishlist.builder().memberId("member-1").book(book).build());

        assertThat(wishlistRepository.findByMemberIdAndBook_BookId("member-1", book.getBookId())).isPresent();
        assertThat(wishlistRepository.findByMemberIdAndBook_BookId("member-2", book.getBookId())).isEmpty();
    }
    ...
    void 삭제된_책은_회원의_찜_목록에서_제외한다() {
        ...
        List<Wishlist> result = wishlistRepository
                .findByMemberIdAndBook_IsDeletedFalseOrderByCreatedAtDesc("member-1");
    ...
    void 찜을_삭제한다() {
        ...
        assertThat(wishlistRepository.findByMemberIdAndBook_BookId("member-1", book.getBookId())).isEmpty();
    }
