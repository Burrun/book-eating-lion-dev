...
| 12 | **✅ WAF `rate-limit`이 단일 IP 부하테스트를 막던 문제 — `lion-team3-integrated`는 해결됨, `lion-team3-dev`는 미해결** (2026-08-28) | `rate-limit`(IP당 5분 2,000건 초과 시 차단) 예외로 `ip-allowlist`(IPSetReferenceStatement, Allow, Priority 0) 규칙 추가됨. **처음엔 `lion-team3-dev`(dev.ajttk.com이 쓰는 WebACL)가 아니라 `lion-team3-integrated`(book.ajttk.com)에 잘못 넣어서 한동안 계속 막혔다** — `aws wafv2 get-web-acl`로 직접 대조해서 확인함. | `book.ajttk.com` 대상 부하테스트는 정상 진행 가능(2026-08-28 01/02번 실측 성공, `capacity-and-cost-guide.md` §6 참고). **`dev.ajttk.com`은 여전히 이 예외가 없어서 그대로 돌리면 막힌다.** | dev.ajttk.com도 테스트하려면 같은 IP를 `lion-team3-dev` WebACL에도 추가해야 함(미결정 상태로 남겨둠 — 필요해지면 요청할 것). |
| 13 | **✅ NLB 공개 노출 보안 이슈 — 해결 확인됨** (2026-08-28 발견, 2026-08-28 수정 확인) | `terraform/modules/compute/ingress_alb`가 NLB를 `internet-facing`으로 설정해서 WAF(rate-limit + OWASP 방어 전부)를 완전히 우회해 오리진에 직접 접근 가능했다(실측 확인: NLB로 직접 50개 동시 요청, 전부 200). | 부하테스트와 무관하게 심각한 보안 취약점이었음 — 실제 공격자도 이 경로로 WAF를 통째로 우회할 수 있었음. | **해결됨.** NLB 보안그룹에 CloudFront 관리형 prefix list(`pl-22a6434b`, "CloudFront origin-facing traffic only")만 허용하도록 제한됨 — 재확인 결과 NLB 직접 접근 시 `code=000`(연결 자체 실패)으로 막힘. |
