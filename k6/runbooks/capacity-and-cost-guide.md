...
|---|---|---|---|---|
| 2026-08-28 | 02-cache-offload | book-postapply | 실패율 0%, p95 1.3s | 캐시 미구현 threshold(p95<10ms)만 예상대로 미달, 나머지 정상 |
| 2026-08-28 | 01-traffic-spike | book-postapply(5000 VU) | 실패율 97.64%, 성공분 p95 45.98s | 완전 실패 |
| 2026-08-28 | 01-traffic-spike | book-spike-1000 | 실패율 93.41%, 성공분 p95 13.56s | 완전 실패 |
| 2026-08-28 | 01-traffic-spike | book-spike-750 | 실패율 0%, p95 462ms | 완전 통과(전 threshold 그린) |
| 2026-08-28 | 01-traffic-spike | book-spike-850 | 실패율 0%, p95 658ms | 가용성 통과, 지연(p95<500ms)만 미달 |

자세한 배경·해석은 §6 참고.

---

## 6. 실측 사례 — book.ajttk.com(integrated) 트래픽 스파이크 한계 탐색 (2026-08-28)

### 사전 조건

- 대상: `book.ajttk.com`(WAF `lion-team3-integrated`) — **팀 확인: 실사용자 없는 테스트
  환경, 부하테스트 진행 동의 받음.** 읽기 전용 시나리오(01/02)만 실행 — 03(pod-kill)/05
  (실주문 생성)/07(카드 발급)은 이 도메인엔 안 돌림.
- 이 테스트를 하려면 사전에 두 가지가 먼저 고쳐져야 했다(README §0-12, §0-13):
  1. WAF `rate-limit`(IP당 5분 2,000건) 예외를 처음엔 **엉뚱한 WebACL**(`lion-team3-dev`가
     아니라 `lion-team3-integrated`인데, 처음엔 반대로 넣었었음)에 넣어서 계속 막혔다 —
     맞는 WebACL에 다시 적용한 뒤부터 정상화됨.
  2. NLB 공개 노출 보안 이슈(§0-13)도 이 시점에 보안그룹으로 막힘 — CloudFront 관리형
     prefix list(`pl-22a6434b`, "CloudFront origin-facing traffic only")로 제한된 것을
     `aws ec2 describe-security-groups`로 직접 확인함.

### 결과

| 시나리오 | 목표 | 실패율 | p95 지연 | 판정 |
|---|---|---|---|---|
| 02-cache-offload | 200 req/s, 1분 | 0% | 1.3s | ✅ 통과 |
| 01-traffic-spike | 5,000 VU | 97.64% | 성공분 45.98s | ❌ 완전 실패 |
| 01-traffic-spike | 1,000 VU | 93.41% | 성공분 13.56s | ❌ 완전 실패 |
| 01-traffic-spike | 750 VU | 0% | 462ms | ✅ 완전 통과(전 threshold) |
| 01-traffic-spike | 850 VU | 0% | 658ms | ⚠️ 가용성 통과, 지연만 목표(500ms) 초과 |

**결론**: 가용성이 완전히 무너지는 경계는 850~1,000 VU 사이. **900 VU가 다음 확인
대상(아직 미실행)** — 계속 좁혀서 정확한 경계를 찾을 것.

### ⚠️ 중요한 방법론적 주의사항 — 순차 실행 오염

500 VU(p95 1.95초)보다 750 VU(p95 462ms)가 오히려 더 빨랐다 — VU를 늘렸는데 더
빨라진 것. **"750이 더 가볍다"가 아니라, 각 테스트를 곧바로 이어서 실행해서 앞선
테스트로 이미 늘어난 HPA replica가 아직 스케일다운되지 않은 채로 다음 테스트를
시작했기 때문일 가능성이 높다.** catalog HPA는 스케일다운을 천천히 하도록 설정돼
있다(`k8s/catalog/hpa.yaml` `behavior.scaleDown`).

**즉 이번에 연속 실행하며 좁혀나간 "경계"는 "완전히 식은 상태(cold start)에서의
순수 한계"가 아니라 "직전 테스트의 잔여 스케일 상태에서의 한계"에 가깝다.** 진짜
콜드 스타트 한계를 알고 싶다면 각 실행 사이에 replica가 최소치로 돌아올 때까지
기다렸다가(`kubectl get hpa -n lion-app -w`로 확인) 실행해야 한다 — 이번 세션에선
시간 관계상 그렇게 하지 않았으니, 이 숫자를 "웜업된 상태에서의 한계"로 해석할 것.
정확한 콜드/웜 양쪽 다 필요하면 재실행해서 비교 기록을 추가할 것.
