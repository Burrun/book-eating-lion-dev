...
# ── 7. catalog-service IRSA (EPUB Presigned URL 발급용) ───────────
# catalog-service는 IRSA가 아예 없어 default ServiceAccount로 떴고,
# S3EbookStorageAdapter의 createReadUrl/createUploadUrl 호출이 SdkClientException
# (자격증명 체인 전부 빈 상태)으로 죽어 EPUB 열람/업로드가 500이었다
# (2026-08-27 dev 실배포에서 실제로 겪음).
...
# ── 8. order-service IRSA (구매 확정 SQS 발행용) ────────────────
